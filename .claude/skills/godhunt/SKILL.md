...
> **Invocation:** in Claude Code, type `/godhunt [market]` (e.g. `/godhunt Turkey`). Or type `/skill godhunt [market]`. Or just describe the request in natural language — the agent will load this skill automatically.
> **Argument** (optional): target market (e.g. `Turkey`, `Brazil`, `Indonesia`, `global`). If omitted, read from `godmode.md` → `## Current User Profile` → `primary_domain`. If still unclear, default to `"global"`.
