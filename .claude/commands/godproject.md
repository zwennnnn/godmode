...
> **Trigger:** user types `/godproject <slug>` (e.g. `/godproject turkce-ai-destek`).
> **Argument** (`$ARGUMENTS`): the project name / folder name under `projects/`. Required.
> **Behavior:** scaffold the actual code (package.json, src/, .env.example) for an existing `projects/$ARGUMENTS/` (typically created by `/godhunt` or hand-written). Fully autonomous — no user input.

## Protocol (agent must follow exactly)

1. **Verify `projects/$ARGUMENTS/` exists** and has a `README.md` (from `/godhunt` or hand-written).
...
4. **Generate the scaffold:**
   ```bash
   python scripts/project.py init --name $ARGUMENTS --framework <nextjs|fastapi|express|react-vite|django|go-gin>
   ```
...
7. **Tell the user:**
   > "Code scaffolded in `projects/$ARGUMENTS/`. `cd projects/$ARGUMENTS && pnpm install && pnpm dev` to start."
